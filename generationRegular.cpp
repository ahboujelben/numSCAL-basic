...
{
    if(networkSource==1)
        assignPoresConductivities();
    else
        assignElementsConductivities();
}

void network::assignPoresConductivities()
{
    for (int i = 0; i < totalPores; ++i)
    {
        pore* p=getPore(i);
...
void network::assignElementsConductivities()
{
    for(int i=0;i<totalNodes;++i)
    {
        node* n=getNode(i);
        if(!n->getClosed())
        {
            n->setNodeConductivity(n->getShapeFactorConstant()*pow(n->getRadius(),4)/(16*n->getShapeFactor())/(n->getViscosity()*n->getLength()));
        }
    }
    ...
    for(int i=0;i<totalPores;++i)
    {
        ...
        if(!p->getClosed())
        {
            ...
            p->setThroatConductivity(1/throatConductivityInverse);
...
void network::assignWettability()
{
    if(networkSource==1)
    {
        assignPoresWettability();
        clusterWaterWetPores();
        clusterOilWetPores();
    }
    else
    {
        assignElementsWettability();
        clusterWaterWetElements();
        clusterOilWetElements();
    }
}

void network::assignPoresWettability()
{
    ...
    if(wettingTypeFlag==3) //FW
    {
        ...
        int  oilWetSoFar=0;
        while((double(oilWetSoFar)/double(totalOpenedPores))<oilWetFraction)
        {
            int index=uniform_int(0,totalPores-1);
            pore* p=getPore(index);
...
void network::assignElementsWettability()
{
    if(wettingTypeFlag==1) //WW
    {
