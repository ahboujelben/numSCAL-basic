...
void network::solvePressuresForExtractedModel()
{
...
//    if(networkSource==4)
//    {
//        solvePressuresForFracturedModel();
//        updateFlows();
...
//                fflow+=p->getFlow();
//        }
//        absolutePermeabilityFracture=(fflow*xEdgeLength)/(yEdgeLength*zEdgeLength*(pressureIn-pressureOut));
//    }
}

void network::calculateRelativePermeabilitiesOW()
{
    ...
    if (isOilSpanning)
    {
        ...
        for(int i=0;i<totalPores;++i)
        {
            pore* p=getPore(i);
            if(!p->getClosed() && p->getOutlet())
                if(p->getPhaseFlag()=='o')oilFlow+=p->getFlow();
        }
    }

    if(networkSource==1 || networkSource==4)
        clusterWaterPores();
    else
    ...
    if (isWaterSpanning)
    {
        ...
        for(int i=0;i<totalPores;++i)
        {
            pore* p=getPore(i);
            if(!p->getClosed() && p->getOutlet())
                if(p->getPhaseFlag()=='w')waterFlow+=p->getFlow();
        }
    }
...
void network::calculateRelativePermeabilitiesOG()
{
    gasRelativePermeability=0;
    oilRelativePermeability=0;

    pressureIn=1;
    pressureOut=0;

    double gasFlow(0),oilFlow(0);
    ...
    if (isGasSpanning)
    {
        ...
        for (int i = 0; i < totalPores; ++i)
            {
                ...
                if(p->getPhaseFlag()=='o')
                   p->setConductivity(1e-100);
                if(p->getPhaseFlag()=='g' && !p->getClusterGas()->getSpanning())
                   p->setConductivity(1e-100);
            }
        solvePressures();
        updateFlows();
        for(int i=0;i<totalPores;++i)
        {
            pore* p=getPore(i);
            if(!p->getClosed() && p->getOutlet())
                if(p->getPhaseFlag()=='g')gasFlow+=p->getFlow();
        }
    }

    oilRelativePermeability=oilFlow*oilViscosity/flow;
    gasRelativePermeability=gasFlow*gasViscosity/flow;
}

void network::calculateRelativePermeabilitiesWG()
{
    gasRelativePermeability=0;
    waterRelativePermeability=0;
...
{
    solvePressuresForExtractedModel();
}
