...
std::string PNM::numscalNetworkBuilder::getNotification()
{
    ...
    ss << userInput::get().rockPrefix << " numSCAL Network / Total Nodes: " << network->totalNodes << " / Total Throats: " << network->totalPores << "\n"
...
{
    std::string filePath = userInput::get().extractedNetworkFolderPath + userInput::get().rockPrefix + "_nodes.num";

    std::cout << "Importing data from " << filePath << "..." << std::endl;

    double x, y, z, radius, length(-1), shapeFactor(0.03), id(0);
    ...
    in.read_header(io::ignore_missing_column, "x(um)", "y(um)", "z(um)", "radius(um)", "length(um)", "shapeFactor");
    while (in.read_row(x, y, z, radius, length, shapeFactor))
    {
        ...
        if (x > network->xEdgeLength)
            network->xEdgeLength = x;
        if (y > network->yEdgeLength)
            network->yEdgeLength = y;
        if (z > network->zEdgeLength)
            network->zEdgeLength = z;
    }
...
{
    std::string filePath = userInput::get().extractedNetworkFolderPath + userInput::get().rockPrefix + "_throats.num";
    ...
    in.read_header(io::ignore_missing_column, "nodeIn(id)", "nodeOut(id)", "radius(um)", "length(um)", "shapeFactor");
    while (in.read_row(nodeInId, nodeOutId, radius, length, shapeFactor))
    {
        ...
        auto nodeIn = nodeInId == 0 ? 0 : network->getNode(nodeInId - 1);
        auto nodeOut = nodeOutId == 0 ? 0 : network->getNode(nodeOutId - 1);
...
void numscalNetworkBuilder::assignMissingValues()
{
    ...
    for (pore *p : pnmRange<pore>(network))
    {
        if (p->getNodeIn() == 0 || p->getNodeOut() == 0)
            if (p->getLength() == -1)
                p->setLength(averageLength);
    }

    network->is2D = network->zEdgeLength == 0 ? true : false;

    if (network->zEdgeLength == 0)
        network->zEdgeLength = averageLength;
}
