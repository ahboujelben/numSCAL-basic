...
Qt::GlobalColor MainWindow::COLOR_LIST[] = {
    Qt::red,       Qt::blue,     Qt::green,       Qt::gray,     Qt::black,
    Qt::magenta,   Qt::yellow,   Qt::gray,        Qt::darkBlue, Qt::darkRed,
    Qt::darkGreen, Qt::darkGray, Qt::darkMagenta, Qt::cyan};
...
void MainWindow::on_loadNetworkButton_clicked() {
  ...
  try {
    ...
    importNetworkDataFromFile();
  } catch (std::exception& ex) {
    std::cout << "A problem has occured.\n" << ex.what() << "\n";
  ...
  std::thread([this]() {
    try {
      ...
      connect(builder.get(), SIGNAL(notifyGUI()), this,
              SLOT(updateNetworkProgressBar()));
      connect(builder.get(), SIGNAL(finished()), this,
              SLOT(updateGUIAfterLoadingNetwork()));
...
void MainWindow::on_twoPhaseSimButton_clicked() {
  if (!networkBuilt || currentlyBusy) return;

  try {
    exportSimulationDataFromGUI();
    importSimulationDataFromFile();
    updateGUIBeforeSimulation();
  } catch (std::exception& ex) {
    std::cout << "A problem has occured.\n" << ex.what() << "\n";
    updateGUIAfterSimulationFailure();
    return;
  }
  ...
  std::thread([this]() {
    try {
      ...
      connect(sim.get(), SIGNAL(notifyGUI()), this,
              SLOT(updateGUIDuringSimulation()), Qt::BlockingQueuedConnection);
      connect(sim.get(), SIGNAL(finished()), this,
              SLOT(updateGUIAfterSimulation()));
      sim->execute();
    } catch (std::exception& ex) {
      std::cout << "A problem has occured.\n" << ex.what() << "\n";
      updateGUIAfterSimulationFailure();
...
void MainWindow::on_renderNetworkButton_clicked() {
  ...
  std::thread([this]() {
    try {
      ...
      sim->setNetwork(network);
      connect(sim.get(), SIGNAL(notifyGUI()), this,
              SLOT(updateGUIDuringRendering()), Qt::BlockingQueuedConnection);
      connect(sim.get(), SIGNAL(finished()), this,
              SLOT(updateGUIAfterRendering()));
      sim->execute();
    } catch (std::exception& ex) {
      std::cout << "A problem has occured.\n" << ex.what() << "\n";
      updateGUIAfterSimulationFailure();
...
void MainWindow::exportNetworkDataFromGUI() {
  if (ui->loadFromFileRadioButton->isChecked()) return;
  ...
  settings.setValue("extractedNetworkPath",
                    ui->pathToExtractedLineEdit->text());
...
void MainWindow::exportSimulationDataFromGUI() {
  if (ui->twoPhaseLoadFromFileRadioButton->isChecked()) return;
  ...
  settings.setValue("secondaryImbibitionSimulation",
                    ui->SICheckBox->isChecked());
  settings.setValue("tertiaryDrainageSimulation", ui->TDCheckBox->isChecked());
  settings.endGroup();

  settings.beginGroup("FluidInjection_SS");
  settings.setValue("twoPhaseSimulationSteps",
                    ui->twoPhaseStepsLineEdit->text());
  settings.setValue("filmConductanceResistivity",
                    ui->twoPhasefilmConductance->text());
  settings.setValue("relativePermeabilitiesCalculation",
                    ui->twoPhaseRPermCheckBox->isChecked());
  ...
  settings.setValue("overrideByInjectedPVs",
                    ui->overrideByInjectedPVs->isChecked());
  ...
  settings.setValue("initialWaterSaturation",
                    ui->twoPhaseInitialSwSpinBox->text());
  int distribution = 1;
...
void MainWindow::importNetworkDataFromFile() {
  PNM::userInput::get().loadNetworkData();
}

void MainWindow::importSimulationDataFromFile() {
...
void MainWindow::updateGUIAfterSimulationFailure() {
  ui->widget->setSimulationRunnning(false);
  ui->simulationProgressBar->setVisible(false);
  ui->twoPhaseSimButton->setEnabled(true);
  ui->twoPhaseSimStopButton->setEnabled(false);
...
void MainWindow::exportNetworkToImage() {
  ...
  std::string imagePath =
      "Videos/IMG" + boost::str(boost::format("%07d") % imageIndex) + ".png";
  image.save(imagePath.c_str());

  imageIndex++;
}

void MainWindow::on_twoPhaseSimStopButton_clicked() { sim->interrupt(); }

void MainWindow::on_renderStopButton_clicked() { sim->interrupt(); }
...
void MainWindow::on_Rcolor_valueChanged(int value) {
  if (ui->oilColor->isChecked()) ui->widget->getOilColor().x = value / 255.;

  if (ui->waterColor->isChecked()) ui->widget->getWaterColor().x = value / 255.;
...
void MainWindow::on_Gcolor_valueChanged(int value) {
  if (ui->oilColor->isChecked()) ui->widget->getOilColor().y = value / 255.;

  if (ui->waterColor->isChecked()) ui->widget->getWaterColor().y = value / 255.;
...
void MainWindow::on_Bcolor_valueChanged(int value) {
  if (ui->oilColor->isChecked()) ui->widget->getOilColor().z = value / 255.;

  if (ui->waterColor->isChecked()) ui->widget->getWaterColor().z = value / 255.;
...
void MainWindow::on_actionAbout_triggered() {
  QMessageBox::about(
      this, tr("About numSCAL"),
      tr("numSCAL is a pore network simulator developed by \nAhmed Boujelben "
...
void MainWindow::on_pushButton_clicked() {
  if (ui->title->text() != "") curvesWidget->setText(ui->title->text());
...
void MainWindow::on_plot_clicked() {
  ...
  if (file) {
    ...
    for (unsigned i = 1; i < vstrings.size(); ++i) {
      QPen pen(COLOR_LIST[totalCurves % 14]);
      pen.setWidth(2);
